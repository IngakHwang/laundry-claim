...
- **클레임 접수** — 거래처·유형(품질/배송/기타)·심각도·사진(여러 장, 폰 카메라 직접 촬영)·담당자 배정·지시
...
Python 3.12 · FastAPI · SQLite · Jinja2(서버 렌더링) · 순수 JS 약간(드래그·미리보기·드릴다운)
...
- SQLite → PostgreSQL 이관 (배포 환경에서 데이터 영속)
- SMS 알림 — `notify()` 함수 자리만 비워둔 상태 (유료·발신번호 등록 절차라 보류)
