...
- **클레임 접수** — 거래처·유형(품질/배송/기타)·심각도·사진(여러 장, 촬영 또는 앨범)·담당자 배정·지시.
  오접수는 접수자 본인이 삭제할 수 있다 — 단, 다른 사람의 기록이 붙기 전까지만
- **칸반 현황판** — 접수 → 확인됨 → 처리중 → 완료. 카드 끌어놓기(데스크톱)·다음 단계 버튼(모바일), 이동도 조치 일지에 자동 기록
- **조치 일지** — 지시(어떻게 해야 하는지)와 처리 보고(어떻게 했는지)가 시간순으로 쌓이는 타임라인.
  본인 글만 수정·삭제할 수 있고, 수정 전 내용과 삭제 흔적(취소선)이 그대로 남는 장부 방식
...
- 표 12장: 공장 · 거래처 · 품목 · 거래처별 품목(N:M) · 담당자 · 클레임 · 조치 · 조치 수정 이력 · 사진(1:N) · 설정(카톡 토큰) · 대화방 · 대화글
...
- **합격선 시험** — 질문 112문 배치 시험을 통과한 버전만 터널에 연결한다 (답변 품질의 회귀 방지)
...
Claude Code와 함께 하루 만에 v1을 만들고, 직접 써 보며 나온 불편부터 20여 회 고쳤다.
