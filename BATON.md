...
- **v2 확정(2026-08-10, 순서대로):** ①UI/UX 점검·디자인 업그레이드(진행 중 — 검증은 수정 이력 모르는 새 눈으로 분리) ②SQLite→Postgres 이관(**카드 등록 없는 무료만** — Neon 등, Render 무료 DB는 30일 뒤 삭제라 제외) ③알림 = 카톡 무료 범위(나에게 보내기)로 notify() 채우기 — SMS 아님
