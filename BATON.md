...
- **v2 확정(2026-08-10, 순서대로):** ①UI/UX 개선 — 점검(41건)→12건 수정→재질 교체(보라+Pretendard)→아이콘·로그인 재구성→**새 눈 재검증 통과(16건 전원 해소)**. 남은 확인 = 인각님 폰으로 데모 열어보기 ②SQLite→Postgres 이관(**카드 등록 없는 무료만** — Neon 등, Render 무료 DB는 30일 뒤 삭제라 제외) ③알림 = 카톡 무료 범위(나에게 보내기)로 notify() 채우기 — SMS 아님
